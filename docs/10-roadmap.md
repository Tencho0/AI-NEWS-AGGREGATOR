...
- [x] **Phase 4b images** (2026-07-03): photo message under each card (top stock suggestion +
      attribution caption), 🖼 cycle button, editor photo upload via reply (downloaded locally,
      wins selection, publishes as base64), reply-binding of instructions/uploads to specific
      cards; migration 0008. Pixabay + Pexels keys provisioned (user-secrets)
- [ ] Commands: `/status /topics /mute /pause /resume` done; `/draft` force-draft and full-text
      attachment → backlog (nice-to-have)
...
| Date | Entry |
|---|---|
| 2026-07-18 | **SEO/Discover pack implemented** in the Predel-News site (promotion strategy item 2; plan `Predel-News/docs/superpowers/plans/2026-07-18-seo-discover-pack.md`, 9 tasks, subagent-driven with per-task review): `/news-sitemap.xml` endpoint (48 h window, current Google News schema — publication/date/title only, deprecated `keywords`/`genres` dropped), NewsArticle JSON-LD rebuilt via a pure Core builder (absolute ≥1200 px 16:9/4:3/1:1 image set, UTC `...Z` dates, org-vs-person author, `DateTime.MinValue`→absent `dateModified`), `max-image-preview:large` robots meta (search page `noindex,follow`), absolute 1200×630 `og:image` + `og:image:alt` + `article:section`/`article:tag`, config-driven `google-site-verification` (`PredelNews:Seo:GoogleSiteVerification`), warn-only sub-1200 px cover-image check in the publishing endpoint, plus a fix for a pre-existing `/sitemap.xml` 500 (missing UmbracoContext). 25 new Core unit tests (177/177 green). Decisions recorded: production domain **predelnews.com** (the `.bg` code strings were bugs), dates treated as UTC. Google Publisher Center submission is obsolete (removed 2024/25 — inclusion is automatic); owner steps in `runbooks/google-search-console.md`. Owner follow-ups: run on a live domain, submit sitemaps in GSC, one-time schema.org/Rich-Results validation, confirm the image-width warning fires live. |
...
| 2026-07-03 | **First organic end-to-end publication.** The M1 collection run promoted its first organically Hot topic (topic 15 «Санкции на ЕС … Навални», 2 sources, score 6.84), DraftJob produced draft 1 with 3 stock image suggestions, the editor approved it in Telegram, and the pipeline published it to the dev site **with the real Pixabay cover image** (387 KB JPEG fetched server-side; verified via `og:image`). The first attempt exposed a real idempotency flaw: the Newsroom DB had been rebuilt for the M1 run, so draft ids restarted while the site's `pn_NewsroomPublish` ledger persisted — `externalRef "newsroom-draft-1"` collided with yesterday's Атанасова entry and the endpoint silently returned the *old* article. Fixed properly: migration `0009_publish_ref` adds a `PublishRef` GUID (default `NEWID()`) to `nw_Draft`; `externalRef` is now `newsroom-{PublishRef:N}` end-to-end (Core record, repository, publisher, tests, docs). 209 tests green; worker redeployed on schema v9, publishing leg returned to dormant for the remainder of the 48 h collection run. |
...
| 2026-07-03 | **Phase 4b (image review UX) implemented + image keys provisioned.** Pixabay/Pexels keys validated and stored (user-secrets); photo message per review card with attribution caption, 🖼 „Друга снимка" cycling (mirrors the publish pick), editor photo upload by replying to a card (stored locally, `Selected=1`, published as base64; missing file = permanent rejection with a clear message), reply-binding fixes multi-draft change-conversations; migration 0008 (`TelegramPhotoMessageId`). 209 tests green; live worker updated. During the same session the trend pipeline organically promoted its first Hot topic (2 sources, score 6.84 > 6.0 threshold) with zero manual help. AI-generated illustrations (Q-4) discussed with the owner: hard no on photo-realistic real-people/events imagery (ADR-0009 reaffirmed with the Атанасова case as the canonical example); stylized labeled illustrations via Gemini image models offered as an optional tier — awaiting owner decision. |
